= 4. Implementation
  ...
  The core for the classical machine learning algorithms implementation relies on Scikit-learn. It provides the modules for data preprocessing (*StandardScaler, OneHotEncoder*), building composite pipelines (Pipeline, ColumnTransformer), training the classical models (LogisticRegression, SVC, DecisionTreeClassifier, etc.) and calculating performance metrics. For visualizing the libary Matplotlib and Seaborn was used.
 ...
 A total of 100 individual training runs were executed by the W&B agent. The goal was to to explore the defined hyperparameter space. The results of this search are visualized in Figure 5, which illustrates the relationship between different hyperparameter values and also the resulting model performance. In general the W&B sweep identifies the optimal hyperparameter to maximaze the final validation accuracy (val_accuracy), which is achieved at the end of training. When i retrained the model, i found it interestingly, that the re-trained model, which was using the supposedly optimal configuratio, had worrse reults than my inferior model with the initial default hyperparameters. After that i  observed the validation accuracy curve for all 100 runs, whihc revealed a crucial insight. One run, which had not the highest final accuracy, achieved the overall peak accuracy of 0.81 at epoch 18 out of 30. 
   #figure(
